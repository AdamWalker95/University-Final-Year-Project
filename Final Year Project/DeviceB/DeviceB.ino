...
void reedOutput()
{
  ...
  obj.DoorReadings(outputTime, activityPresent);//outputs activity report
  if (activityPresent == obj.DoorReadings(outputTime, activityPresent))
  {
    String startTime = outputTime;//records time when activity begins
  }
  else
  {
